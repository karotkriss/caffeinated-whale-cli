...
3b. **Real-Docker E2E for multibench status/start against GENUINELY SERVING benches** - `tests/e2e/test_multibench_serving_e2e.py` (standalone, v16 leg). It builds a real second bench - its own `bench init` virtualenv, site, Procfile, supervisord and assigned port - because the multibench test in 3a builds a directory SKELETON, which can pin the document shape and nothing about the web probe. The three defects `report-status-per-bench` fixed only exist when a second bench genuinely answers on its own port: a healthy bench past the first reported `degraded` (F3), a dead one reported its neighbour's live HTTP code (F4), and `start --bench N` for N>0 waited on `:8000` and warned falsely (F5). Each test asserts the POSITIVE (this bench is genuinely serving) before the negative, and asserts the DISCRIMINATOR - the value the removed bench-blind probe would have read - so a regression to `:8000` cannot pass. `tests/e2e/test_multibench_latency_e2e.py` is its opt-in sibling carrying `report-status-per-bench` task 5.5's six-bench latency measurement (`CWE2E_LATENCY_BENCHES=6`, or the `latency_benches` input on the E2E workflow); numbers in `docs/e2e/multibench-serving-status.md`.
...
4p. **Host-uid alignment for the workspace bind mount, the CI `cwcli rm` permission-denied root-cause fix** (`core/docker.py`) - bench commands exec as the frappe/bench image's default `frappe` user (uid 1000), so files written to the `../data` bind mount land host-owned by uid 1000; a host uid that differs (a CI runner is 1001) then cannot recurse-delete them and `cwcli rm` failed with `[Errno 13] Permission denied`.
`core.docker.align_container_user_to_host` remaps the container `frappe` user to the host uid/gid (the frappe devcontainer's `updateRemoteUserUID` trick); `init_bench` calls it with `chown_home=True` before any provisioning exec (a one-time `/home/frappe` chown so pyenv/nvm/pip installs can write it), and `core.start` re-aligns cheaply (usermod/groupmod only) on every launch so a container recreation (which resets `frappe` back to uid 1000) is covered without another steady-state call site.
`core.scale` also calls it conditionally when repairing old-major toolchains after a recreation; the `core/scale.py` coverage entry above owns that specialized path.
A no-op when the ids already match; a failed remap degrades to a soft warning (`init.uid_align_failed`/`start.uid_align_failed`, both rendered unconditionally, not gated behind `--verbose`) and the bench still builds/starts owned by the original uid.
Covered by `test_core_docker.py` (the no-op fast path, the uid+gid remap with/without the home chown, a failed remap or unreadable ids degrading to a warning rather than a raise); the `id -u`/`id -g frappe` probe fake plumbing in `test_core_init.py`/`test_core_supervision.py` keeps the align step a no-op in those existing suites.
See the host-uid alignment note in the `cwcli-lifecycle` skill's `init.md` reference.
