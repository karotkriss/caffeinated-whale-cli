...
`test_unlock_e2e.py`, `test_label_e2e.py`,
...
Current overall coverage at 1.0.0 (1682 tests across 92 test files, ~82% overall).
...
- ✅ `core/bench_ops.py`, `cwcli axi migrate` / `cwcli axi run-tests` - the two execution verbs. `test_core_bench_ops.py` pins the decisions rather than the implementation: a migrate resolves EXACTLY ONE site and never fans out (the module's whole reason to exist, since `apps update` discovers its targets); the maintenance gate REFUSES rather than warns, so a site that cannot enter maintenance gets no `bench migrate` at all; the disable runs in a `finally` even when the migrate raises (why this is a plain function and not a generator - the cost of skipped cleanup is a site left DOWN); a site left in maintenance is reported and fails; `run_tests` takes its site and app as REQUIRED parameters, pinned at the CORE so no frontend can default them; and `set_maintenance` is proven SHARED with `core.update` rather than copied. `test_axi_bench_ops.py` covers the frontends: one TOON document with the resolved site readable back, exit from `report.ok` so a WARNING-shaped envelope still exits 1, `maintenance_left_on` surfaced in the document, the missing-`--site`/`--app` usage errors (exit 2, before any Docker call), the `confirm_start`/`select_bench` forks as exit-2 usage errors, the command's own bytes on stderr and never stdout, a help line on failure but not on success (AXI §9), and two registry assertions - both verbs are TOP-LEVEL with no `bench` group, and neither has a variadic or free-form parameter through which a command string could be expressed. See `openspec/changes/add-axi-bench-exec-verbs`.
- ✅ `core/apps.py` (~98%), the reseated `commands/apps.py` renderer (~98%) - `list_apps`/`install_apps`/`uninstall_apps` joined `core.update` on the logic core, so the module no longer carries two contracts. `test_apps_characterization.py` is the green-before net (written and committed against unmigrated code, `commands/apps.py` 91.28% -> 99.49%); `test_core_apps.py` covers what only `axi`/a future GUI can reach - the pre-resolved `confirm_start`/`select_bench`/`confirm_uninstall` forks, the no-cache default, and that the core prints nothing and returns plain serializable data; `test_axi_apps_list.py` covers the new `cwcli axi apps list` verb (TOON, exit 0/1/2, the null-vs-empty-list distinction on a failed site read, and that `install`/`uninstall` are deliberately unregistered as axi verbs); `test_axi_apps_checkout.py` covers `cwcli axi apps checkout` (one TOON document with a row per git step, exit from `report.ok` so a refused checkout exits 1, the `confirm_start`/`select_bench` forks as exit-2 usage errors, git's own bytes narrated to stderr and never stdout, the recache epilogue including its warn-but-exit-0 failure, and that `--reset` is the only path that discards local work). See `openspec/changes/migrate-apps-core` and `openspec/changes/add-axi-apps-checkout-verb`.
...
- ✅ `core/docker.py` - ~76% (`test_core_docker`: the host-uid alignment `align_container_user_to_host`/`_read_frappe_id` - the no-op-when-matched fast path, the uid+gid remap with/without the home chown, a failed remap or unreadable ids degrading to a soft warning rather than a raise; `get_project_containers`/`get_project_volumes`/`get_container`'s real `docker.from_env()` DockerException branches remain only incidentally covered, unchanged by this addition)
...
- ✅ `commands/axi.py` `config` verb - (`test_axi_config`: one TOON document carrying every store (nested `auto_inspect` dict, inline `search_paths`), exit 0/1 mapping, the no-arguments/no-flags pure-read signature, and the registry assertions that no config-mutating axi verb exists anywhere - the `axi apps install`/`uninstall` deferral discipline)
...
4p. **Host-uid alignment for the workspace bind mount, the CI `cwcli rm` permission-denied root-cause fix** (`core/docker.py`) - bench commands exec as the frappe/bench image's default `frappe` user (uid 1000), so files written to the `../data` bind mount land host-owned by uid 1000; a host uid that differs (a CI runner is 1001) then cannot recurse-delete them and `cwcli rm` failed with `[Errno 13] Permission denied`. `core.docker.align_container_user_to_host` remaps the container `frappe` user to the host uid/gid (the frappe devcontainer's `updateRemoteUserUID` trick); `init_bench` calls it with `chown_home=True` before any provisioning exec (a one-time `/home/frappe` chown so pyenv/nvm/pip installs can write it), and `core.start` re-aligns cheaply (usermod/groupmod only) on every launch so a container recreation - which resets `frappe` back to uid 1000 - is covered without a third call site. A no-op when the ids already match; a failed remap degrades to a soft warning (`init.uid_align_failed`/`start.uid_align_failed`, both rendered unconditionally, not gated behind `--verbose`) and the bench still builds/starts owned by the original uid. Covered by `test_core_docker.py` (the no-op fast path, the uid+gid remap with/without the home chown, a failed remap or unreadable ids degrading to a warning rather than a raise); the `id -u`/`id -g frappe` probe fake plumbing in `test_core_init.py`/`test_core_supervision.py` keeps the align step a no-op in those existing suites. See the host-uid alignment note in the `cwcli-lifecycle` skill's `init.md` reference.
