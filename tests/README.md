...
- ✅ `core/docker.py` - ~76% (`test_core_docker`: the host-uid alignment `align_container_user_to_host`/`_read_frappe_id` - matching ids with and without the narrowed home repair, the platform no-op without `os.getuid`, uid/gid remapping, and failures degrading to soft warnings; `get_project_containers`/`get_project_volumes`/`get_container`'s real `docker.from_env()` DockerException branches remain only incidentally covered, unchanged by this addition)
