...
- ✅ `core/rm_bench.py` + the human and `axi` renderers - `test_core_rm_bench` covers the running-bench refusal before any consent, core-owned consent (`confirm_remove_bench`), the per-site `drop_site` composition, the fail-closed rule that the bench directory is deleted only once every site's archive lands on the host, the `_refuse_unsafe_bench_path` guard against a <2-segment `rm -rf` target, and the plain-data `BenchRemovalOutcome`. `test_axi_rm_bench` pins the agent surface's required consent-only `--yes`. `tests/e2e/test_rm_bench_e2e.py` proves both frontends against a real bench, including that the backup holds the seeded data before deletion, the bench directory and its database are gone, sibling benches survive and still serve, and a running bench is refused.
...
- ✅ `core/docker.py` - ~76% (`test_core_docker`: the host-uid alignment `align_container_user_to_host`/`_read_frappe_id` - matching ids with and without the narrowed home repair, the platform no-op without `os.getuid`, uid/gid remapping, and failures degrading to soft warnings; the ROOT-HOST fix for #229 - `resolve_frappe_alignment_ids`' decision table (non-root host, root-host fallback to the image default, an explicit override winning over both, the shared service uid winning over a root host, a half-provisioned shared box on root still falling back, and the non-POSIX no-op), align never emitting a remap to uid 0 on a root host, and `align_bind_mount_source_owner` chowning the host data dir to the target/override, staying idempotent on a retry, no-op on a normal host, and skipped in shared mode; `get_project_containers`/`get_project_volumes`/`get_container`'s real `docker.from_env()` DockerException branches remain only incidentally covered, unchanged by this addition). The root-host init is proven against real Docker by `tests/e2e/test_root_host_init_e2e.py` (the pre-fix uid-0 collision forcing `HOME=/root` and the exit-127 PATH breakage, the fix keeping `frappe` usable, and `--uid` remapping to the chosen uid); the CLI surfaces and the two init seams are covered by `test_init_root_uid.py` and `test_axi_init.py::TestUidOverride`
...
- ✅ `commands/axi.py` `benches`/`label`/`self-update` verbs - (`test_axi_label`: TOON `BenchList`/`LabelOutcome` rendering, per-row existence state, document verification state, the `--no-verify` path, exit-code mapping, `axi benches`' uninspected-project error naming inspect rather than an empty list (since `migrate-inspect-core` the axi frontend re-points that hint at `cwcli axi inspect`; `test_axi_inspect` pins the re-point), `axi label`'s required-and-exclusive `--set`/`--clear` (neither is a usage error pointing at `axi benches`, NOT an implicit list), the multi-bench `select_bench` usage error, the `NOT_RUNNING` hint that does not name a flag `label` lacks, and `axi self-update --check`'s deliberate exit-0-when-outdated divergence from the human `--check`'s exit 1)
...
- ✅ `core/credbridge.py` - ~99% (`test_core_credbridge`: host dispatch by the `host=` field to `gh`/`glab`, the context manager's stand-up/answer/teardown over a fake container including teardown on both setup failure and a raise from the wrapped op, the accept loop surviving repeated `settimeout` wakes, and two overlapping bridges against the same bind mount keeping independent per-invocation socket/shim/git-config entries). The AF_UNIX-transport tests are `skipif(os.name == "nt")` (they connect over the `.sock` file); the loopback-TCP tests run on every platform - `_prefer_tcp` picking the transport by host OS, the per-invocation token gate answering only a correctly-prefixed request, the loopback-only bind, and the real generated shim driven end-to-end over `127.0.0.1`. Wrapped by `core.apps.install_apps`'s get-app fan-out, `core.update.update`'s whole dispatch, and both `cwcli run` exec paths (`commands/run.py:_credential_bridged`, with the persistent-daemon skip - `test_core_run.py::TestRunWrapsTheCredentialBridge` pins both paths entering the bridge, the daemon-skip standing up no second helper, the ensure call site, and teardown on a dying stream); run on `windows-latest` by the `Pytest (Windows, native)` CI job (the TCP transport's only real-Windows-kernel coverage). The real proof against live GitHub/GitLab repos was run by hand, not committed as a harness test - but the credential PATH is now committed: `tests/e2e/test_cred_bridge_e2e.py` (v16 leg, shared instance, a PATH-shimmed fake host `gh`) proves the persistent bridge end to end (shim -> stable socket -> daemon -> host tool -> in-container `git credential fill`, audit line credential-free), the §5.8 both-modes degradation (a stopped daemon with shim+config still in place: pty git reaches git's OWN prompt; `GIT_TERMINAL_PROMPT=0` fails clean, nonzero, zero shim noise), the container-recreation self-heal on the next `cwcli start`, and the `run` wrap in both bridge states (ephemeral helper live during the exec and torn down after; no second helper line under the daemon)
- ✅ `core/version.py` - ~90% (`test_core_version`: install-method detection tree (dev/uv/uvx/pip fallback), the fail-open PyPI lookup, PEP 440 compare including the dev-ahead case, the TTL cache, the doctor's no-fetch `read_cached_only`/`current_version` reads, and the `passive_notice` cache-only gate incl. the `attempted_at` once/day refresh throttle)
- ✅ `core/doctor.py` + `commands/doctor.py` + `commands/axi.py` `doctor` verb (`test_core_doctor`: exact in-code registry, closed severity set, every probe classification, read-only tripwires, aggregation, and warn/fail exit aggregate; `test_doctor_characterization`: grouped human rendering and root-callback read-only behavior; `test_axi_doctor`: one TOON document, machine-readable row status, and the same report-driven exit contract)
...
- ✅ `core/url.py`, `cwcli axi url` - the host URL a bench serves on plus a fresh HTTP probe, on demand. Adds no second URL resolver or probe mechanism: it orchestrates `resolvers.resolve_host_web_url` and `supervision.web_http_code` directly, never through `core.status`. `test_core_url.py` pins every branch against a container fake (the resolved URL + fresh code, a non-200 code still reachable, an unreachable port honestly `reachable=False`, the `confirm_start`/`select_bench` `NEEDS_CHOICE` forks shared with single-bench verbs such as `axi logs`, an unreadable port config reported `null`/warned rather than guessed, an explicit `--bench`/`--site` reaching exactly the named bench/site, and a bench with no site at all probed host-less). `test_axi_url.py` covers the frontend rendering and exit mapping. `tests/e2e/test_axi_url_e2e.py` proves the single-bench case against the shared session instance, cross-checked against an independent `docker port` read and a direct `curl` made outside cwcli entirely; the multi-bench discriminator proof (each bench's URL/probe targets its OWN Docker-published port, never a neighbour's) is appended to `tests/e2e/test_multibench_serving_e2e.py`, reusing that module's already-built two-genuinely-serving-benches fixture.

### Modules Needing Dedicated Suites
- ⚠️ `utils/port_utils.py` (~9%, only incidental coverage)
- ⚠️ `utils/docker_utils.py` (`test_core_resolvers` covers the `get_frappe_container` CLI wrapper, and `test_docker_utils` covers the WSL Docker Desktop diagnosis plus the POSIX/Windows `exec_into_container` handover split. The remaining `handle_docker_errors` branches are still only incidentally covered. The UI-free primitives `utf8_stream_decoder`/`get_project_containers`/`get_project_volumes` MOVED to `core/docker.py` so the UI-pure core never imports this UI module - `test_exec_stream_decode.py` dedicated-tests `utf8_stream_decoder` there now. `decode_exec_stream` is GONE: `core/exec_stream.py` superseded it and its only three callers were re-pointed at the primitive)
...
- ✅ `commands/label.py` - (`test_bench_label_db_and_command`: the pre-migration command tests, with list mode updated to verify cached bench existence and the two clear-path consistency tests retained; `tests/e2e/test_label_e2e.py` drives the two-store invariant against a real container, non-interactive only because `label` has no prompt)
...
4b. **Passive update notice** (`update_notice.py` ~95%, wired once into `main.py`'s root Typer callback and bypassed only for `cwcli doctor`/`cwcli axi doctor`) - covered by `test_update_notice` (stderr-only, TTY-gated, `CWCLI_NO_UPDATE_CHECK`-suppressible, fail-open), `test_core_version`'s `TestPassiveNotice` class (the cache-only hot path, the detached background refresh, the `attempted_at` once/day throttle on persistent failure), and `test_doctor_characterization` (neither doctor surface can launch the background cache writer).
...
4d. **`label` onto the logic core, plus `cwcli axi benches`/`cwcli axi label`/`cwcli axi self-update --check`** (`core/label.py` ~98%) - split into `list_benches`/`set_label`/`clear_label` rather than one function. Cached existence verification is shared through `resolvers.present_bench_paths`; each listed row is `present`, `absent`, or `unverified`, and `--no-verify` explicitly chooses the last state. Covered by `test_core_label` (every branch, including the marker-before-cache clear ORDER asserted directly), `test_bench_label_db_and_command` (the command surface and its verified list mode), and `test_axi_label` (the `axi benches`/`axi label`/`axi self-update --check` verbs). `tests/e2e/test_label_e2e.py` covers the DB/marker two-store consistency invariant against a real container, and `tests/e2e/test_stale_bench_cache_e2e.py` covers repeated reads after a real bench removal. See `openspec/changes/migrate-label-core`.
...
4p. **Host-uid alignment for the workspace bind mount** (`core/docker.py`) - covered by `test_core_docker.py` (matching ids with and without the narrowed home repair, the platform no-op without `os.getuid`, uid/gid remapping, and failures degrading to warnings rather than raises).
...
10. **Remaining Real-Docker E2E gaps** are tracked in [`openspec/changes/rebuild-e2e-test-suite/tasks.md`](../openspec/changes/rebuild-e2e-test-suite/tasks.md).
    The current landed proof is summarized in the coverage ledger above.
