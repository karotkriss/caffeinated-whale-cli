...
- **`cwcli start`** - Re-seated on the logic core (`core.start`) and now **idempotent**: re-running against an already-running bench reports "already running" and does nothing, instead of spawning a second `bench start`/honcho stack (the old `pkill -f 'bench start'` never matched honcho's steady-state process, so a re-run double-started). On a multi-bench project with no `--bench` it now prompts which bench interactively and refuses (non-zero) on a non-TTY, matching the rest of the command family (previously it silently used the first bench)
- **`cwcli status`** - Re-seated on the logic core (`core.status`) and enriched from a single blind `curl` into real per-process health read from one `ps`: per-Procfile-process up/down with PID, uptime, CPU%, and RSS, plus the web HTTP probe. stdout stays a single aggregate token (now one of `offline`/`online`/`running`/`degraded`, adding `degraded` for "started but the supervisor is down or the web is not answering"); the per-process breakdown prints on stderr, and the full structured report is available via `cwcli axi status`
- **`cwcli logs`** - The captured `bench start` stream moved from the ephemeral `/tmp/bench-<project>.log` to `<bench>/logs/bench-start.log` on the workspace volume, so it survives a container restart and is size-bounded. `cwcli logs` reads that single-source-of-truth path and gains a `--bench` selector for multi-bench projects
