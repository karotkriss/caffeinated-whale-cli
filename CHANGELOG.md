...
- **`cwcli config paths prune`** - drops custom search paths that are absent from every live instance; dry-run by default (lists what it would remove), `--yes` to apply, `--json` supported, and fails closed if any instance cannot be checked.
- **A hint when a bench is not listed** - `cwcli label` and `cwcli axi benches` now nudge toward `cwcli inspect <project> --update` on an empty or possibly-incomplete bench listing, so a hand-made bench is easier to discover.
- **A one-line guidance hint from the per-invocation credential-bridge shim** - when its socket is gone, raw git now sees "this helper only serves cwcli-run operations; use `cwcli run` or enable the persistent bridge" instead of a Python traceback.

### Changed

- **`cwcli rm` now refuses when a hand-made bench holds sites the backup step never covered** - previously such a bench could be silently deleted with no backup, because the cache-derived backup never knew to target it; `rm` now fails closed in that case and names the way out (`cwcli inspect <project> --update`, then re-run, or the explicit `--no-backup` override).
...
- **A killed terminal or `kill` no longer leaves a site stuck in maintenance mode or leaks credential-bridge leftovers** - SIGTERM and SIGHUP now unwind exactly like Ctrl+C, so the existing cleanup in `migrate`, `apps update`, `apps checkout`, `apps install`, `run-tests`, and `restore` runs on all three signals instead of only SIGINT. For `migrate` and `apps update`, an interrupt now also ends the exact in-container `bench migrate` process cwcli started before clearing maintenance mode as the last step (with a short settle and re-check), closing a race on Frappe v16 where the orphaned migrate could turn maintenance back on after cwcli had already cleared it; under `apps update --skip-maintenance` an interrupt now restores each affected site to its own pre-migrate maintenance state instead of blindly clearing it. Verified deterministic on v14, v15, and v16.
...
- **`cwcli rm` of a site-less (half-provisioned) instance now proceeds in one run** - a live site census recognizes when the backup step found zero sites on any bench and lets removal proceed instead of refusing and requiring a second `--no-backup` run; a blank or invalid project name is now rejected up front and rendered as `(empty)` rather than an empty quoted string.
