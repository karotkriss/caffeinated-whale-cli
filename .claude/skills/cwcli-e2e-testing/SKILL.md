...
It cost real time once already: `test_axi_apps_install` installed an app on the shared site and left it, the bench could not import that app, `frappe schedule` then crashed on its tick and supervisord restarted it about once a minute for the rest of the job, and `test_restart_process_cycles_one_leaves_siblings` - which asserts a sibling's pid is unchanged across a `restart --process web` - failed whenever a flap landed inside its read window.
That assertion had been passing on luck, because in the old single-job tier several minutes of unrelated tests sat between the mutation and the assertion.
