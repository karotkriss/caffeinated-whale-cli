...
description: >
  ...
  Use this whenever you edit or debug commands/init.py, commands/rm.py, commands/restore.py, or
  utils/bench_sites.py - each note guards a real shipped bug, so keep the root-cause "why" so a
  later change does not silently re-break the fix.
---
