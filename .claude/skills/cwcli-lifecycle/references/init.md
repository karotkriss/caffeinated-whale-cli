...
The mount rewrite alone is not enough to make a custom `--bench-parent` resolve correctly: `status`/`run`/`restart`/`backup`/`restore` all resolve the bench path from cwcli's SQLite cache, and `init_bench` only CLEARS that cache (a stale entry is worse than none) - nothing else repopulates it, so an empty cache falls back to the hardcoded default bench path, which only happens to match a bench built under the DEFAULT `--bench-parent`. (`backup`/`restore` no longer fall back BLINDLY - since `fm/cwcli-backup-restore-autoinspect` they auto-inspect a cold cache to discover the real bench first, via `commands.utils.resolve_bench_path_with_fallback` / `core.inspect.resolve_bench_with_fallback` - but the post-init recache below still saves them the round-trip, and `status`/`run`/`restart` still fall back blindly.) Both frontends recache immediately after a fresh bench is created - `commands/init.py:_refresh_cache` (human CLI) and the equivalent `cache.recache_project` call in `commands/axi.py:axi_init` - each degrading a recache failure to a stderr-only warning (never failing init, since the bench itself already succeeded, and never reaching `axi`'s stdout, preserving its one-TOON-document contract).
...
The re-own set is each bench dir PLUS the real source dir of any app symlinked OUTSIDE it (v3.1.2, `fm/cwcli-apps-update-force-ownership`): a devcontainer/externally-provisioned bench points `apps/<app>` at a shared tree like `/workspace/.hdsrc/<app>`, which a `chown -R <bench>` never follows (it changes the link, not its target), so those app repos stayed owned by the pre-shared uid and the next `apps update` git pull hit git's "dubious ownership" refusal.
`_external_app_source_dirs` resolves each `apps/*` symlink and returns only out-of-bench targets, which join the same mismatch-gated re-own so the bench dir already being correct never hides a stranded app repo; `apps update --force` also fixes this inline via `reown_app_repo_to_frappe` without a prior restart (see `.claude/skills/cwcli-apps-update/SKILL.md`).
...
- `--frappe-url <url>` - an optional custom Frappe repo (a fork) threaded through the ONE shared decision point `core.init_bench(frappe_url=...)` onto bench's own `bench init --frappe-path <url>` (purely additive: omitted emits no `--frappe-path` and builds upstream `frappe/frappe` exactly as before). The resolved `frappe_ref` above is the branch/tag checked out WITHIN the fork, so no new branch flag was added - `--frappe-branch` pairs with the URL. A custom fork may be PRIVATE, so the `bench init` clone exec is wrapped in `core.credbridge.credential_bridge(frappe_container, bench_full_path)` - the same context `apps` install/update use - but ONLY when `frappe_url` is truthy; the upstream default (`frappe_url is None`) uses `contextlib.nullcontext()`, leaving the common path provably untouched. The bridge is inert for a public fork too (git only calls a credential helper on a 401). The container is already up at this point (stage 1 `init_instance` brought it up), so the clone is bridgeable exactly like the get-app fetch. Pinned by `tests/test_core_init.py` (bridge wraps the `--frappe-url` exec; default path never stands it up) and the hermetic `tests/e2e/test_init_e2e.py`.
...
Regression coverage: `tests/test_core_init.py::TestInitInstance` pins the healthy skip, partial-stack recovery, and stopped-frappe paths; `tests/test_init_characterization.py::TestAlreadyRunningNotice` pins the human verbose notice; `tests/e2e/test_init_bench_add_preserves_running_e2e.py` proves on real Docker that a bench add leaves the frappe container's id and `StartedAt` byte-identical and the first bench still answering, with no manual restart in between - its first test uses `--no-start` on the add to isolate this container-recreation property in isolation, and its second test repeats the same proof under the DEFAULT `--start` behavior (the new bench's own `core.start` supervisord launch also runs), which `test_multibench_serving_e2e.py`'s `two_benches` fixture exercises but cannot itself prove this property for, since its `_serve_both` precondition would silently restart a first bench this defect had stopped.
