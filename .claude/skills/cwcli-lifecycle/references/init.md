...
The fix is at the SOURCE, not a reclaim/chown-at-`rm` band-aid: on hosts that expose `os.getuid`, `core.docker.align_container_user_to_host(container, *, chown_home=False)` remaps the container's `frappe` user to the HOST uid/gid as root (the frappe devcontainer's own `updateRemoteUserUID` trick), so every file the aligned user writes afterward is already host-owned on any host uid. The gid change is `groupmod -o -g <gid> frappe` (verified cheap - it only rewrites `/etc/group` and any `/etc/passwd` row whose primary gid matched the old value, never walking the filesystem). The uid change is a direct `sed` edit of `/etc/passwd`'s uid field, NOT `usermod -u`: shadow-utils' `usermod -u` unconditionally chowns the target's whole `$HOME` as a documented side effect of a uid change (measured ~90s / a full 1.28 GB overlayfs copy-up on a real image, reproduced twice - `fm/cwcli-uidgid-align-fix`, 2026-07-31), which is the ACTUAL cost an earlier investigation had misattributed entirely to the separate home chown below. The `sed` edit produces byte-identical `/etc/passwd`/`id frappe` output to what `usermod` produced, without touching the filesystem; `usermod` reappearing here silently reintroduces the walk.
...
The two steady-state call sites have deliberately different costs: `init_bench` calls it with `chown_home=True` BEFORE any provisioning exec (right after the reuse-bench decision, before `_ensure_directory`) so the now-remapped user's pyenv/nvm/pip installs during this first provision can still write its own home - narrowed to the specific paths a first provision writes (`_CHOWN_HOME_RECURSIVE_DIRS`/`_CHOWN_HOME_SHALLOW_DIRS` in `core/docker.py`; the 34.6k-file baked pyenv/nvm toolchain beneath them is never re-owned), ~2.5s combined with the uid/gid remap on a mismatched-uid throwaway container (was ~92s before this narrowing plus the `usermod`-to-`sed` fix above); `core.start` calls it on every launch WITHOUT `chown_home` (cheap: the `sed` uid edit and `groupmod` only) so a container recreation - which resets `frappe` back to the image's default uid 1000 - is re-aligned before supervisord writes its per-process logs to the host-owned bench.
