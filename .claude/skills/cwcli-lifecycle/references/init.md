...
Regression coverage: `tests/test_core_init.py` (the tri-state matrix, the choice surfaces, the bounded poll, the auto_start=True fail-closed cap), `tests/test_init_reuse_bench.py` (the frontend prompt loop, prompts-after-spinner-close, the non-TTY refusal), `tests/test_init_characterization.py` (both refusals end-to-end at the command level).
...
All 10 provisioning execs now flow through `core.exec_stream` (bench init, 4 set-configs, new-site, 2 final configs, the ERPNext pair - the audited count; the contract docstring's "11" was an estimate), so the exit code is the contract's honest, bounded-polled one, and a lost stream raises the typed `CwcliError(DOCKER, "exec.stream_lost")` - the batch's one disclosed error-path hardening (both paths exit non-zero).
The ENOSPC hint ("No space left on device inside the container...") stays exactly as alive as before: present on DRAINED execs (the joined output feeds the check), dead on live-rendered ones (`stream_output=True`, the verbose long execs) - restoring it there is the batch-3 audit's recorded non-item (a contract change, not an init change).
