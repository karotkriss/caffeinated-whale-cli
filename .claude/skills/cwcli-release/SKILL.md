...
- **A note can name its flagship entry with `<!-- flagship -->`, and `release-body.sh` is where that guarantee lives.** Put the marker flush left, on its own line, directly above the entry's bold sentence (no blank line between them) - `.github/release-notes/README.md`'s "Naming the flagship entry" section owns the full authoring rule. The script (not a hand-editable template) enforces it before anything publishes: it strips the marker from the published card, refuses a buried/duplicated/detached/malformed marker with an actionable error rather than silently reordering hand-written Markdown, and requires the marker on every **major** release (`X.0.0`) so the headline is stated, not guessed from entry order. A minor/patch release has no such requirement - most have no flagship at all, and an unmarked note still publishes unchanged. Regression coverage is `tests/test_release_body.py::TestTheFlagshipMarker`.
