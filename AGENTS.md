...
#### `update.py` control flow + maintenance-mode safety (u4/b8, 2026-07-11)
...
- Migration runs only over `sorted(maintenance_sites)` (or `sorted(all_affected_sites)` when `--skip-maintenance`): **a site that could not enter maintenance is never migrated** (skip, not a fatal error).
