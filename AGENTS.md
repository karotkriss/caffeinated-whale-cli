...
- The publish step runs `uv publish --trusted-publishing automatic --check-url https://pypi.org/simple/caffeinated-whale-cli/` inside the job's `ghcr.io/astral-sh/uv` container.
  It authenticates via GitHub OIDC trusted publishing (the `id-token: write` permission plus the `pypi` `environment:` block in the workflow), so there is NO `PYPI_API_TOKEN` secret to manage, and `--check-url` skips files already on PyPI instead of failing so re-running an existing version is a no-op.
