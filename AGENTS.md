...
- Every shell interpolation in `update.py` is `shlex.quote`d (site/app/path); docker-py `shlex.split`s string commands, so quoting yields valid tokens (the `sh -c "..."` `test -d` checks keep string form via `_dir_exists`, verified to round-trip through `shlex.split`).
