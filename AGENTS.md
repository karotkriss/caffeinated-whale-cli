...
- **Known simplification:** `cwcli backup -v` on SUCCESS no longer echoes bench's own success chatter (the exec moved into `core.backup`, which returns a DTO, not raw output). The FAILURE path still surfaces bench output via `CwcliError.detail`. Marked with a `ponytail:` comment in `commands/backup.py`; re-add a capture channel only if the success chatter is actually wanted.
