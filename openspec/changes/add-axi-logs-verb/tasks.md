...
firstmate sequences it. These tasks are the plan, not yet executed.

## 1. Baseline pinned before anything moves

- [ ] 1.1 Record the `tests/test_logs.py` + `tests/test_core_logs.py` baseline (pass count, `core/logs.py`
      coverage) so the resolve extraction is proven behaviour-preserving.
- [ ] 1.2 Confirm the five exit-code regressions PR #83 pinned (`test_logs.py:178-217`) are green and stay
...
- [ ] 2.1 Extract a private `_resolve_log_files(project_name, *, bench, bench_path, process, auto_start)`
...
- [ ] 2.2 Reseat `logs_plan` to call `_resolve_log_files` and wrap the result into `LogsPlan` (adding
...
- [ ] 2.3 Run the step-1 baseline: `test_logs.py` + `test_core_logs.py` green, unchanged.

## 3. `core.read_logs`

- [ ] 3.1 Add `ProcessLog` (`process`, `file`, `lines`) and `LogsRead` (`project`, `container_name`,
...
- [ ] 3.2 `read_logs(project_name, *, bench=None, bench_path=None, process=None, lines=100,
...
- [ ] 3.3 Decode with the module's existing decoder (`supervision._decode`), split on the `==> <path> <==`
      headers, map each path back to its program via the resolve, build the `ProcessLog` groups
      oldest-first.
- [ ] 3.4 Running-but-quiet (manager up, no files): return `Result(OK, LogsRead(..., logs=[]))` with a
...
- [ ] 3.5 PURE READ: never launch/install/restart. Stopped container -> `confirm_start` `NEEDS_CHOICE`
      via the shared resolve. Unknown `--process` -> `select_process` `NEEDS_CHOICE`.
- [ ] 3.6 Docstring: state that the bounded tail runs in the core (the `backup` buffered-`exec_run`
      shape) while `logs_plan`'s follow tail stays in the frontend, and why `--follow` is refused here.

## 4. `cwcli axi logs` verb

- [ ] 4.1 Add `axi_logs(project, bench=None, lines=100, process=None)` to `commands/axi.py`: `--bench`,
      `--lines/-n` (default 100), `--process/-p`. NO `--follow`, NO `--yes`.
- [ ] 4.2 Call `core.read_logs`; on `CwcliError` -> `emit_axi_error` + `exit_for(kind)`; on `NEEDS_CHOICE`
      -> `emit_axi_choice_as_usage_error` + exit 2 (handles `select_bench`, `select_process`,
      `confirm_start` with the existing messages).
- [ ] 4.3 On OK, render explicitly (NOT `emit_result`): the metadata head as `toon.kv` lines, then one
...
- [ ] 4.4 Empty read (`logs == []`) still exits 0 with the metadata head and the `logs.none_yet` note.

## 5. Tests

- [ ] 5.1 `tests/test_core_logs.py`: cover `read_logs` with an `exec_run`-interpreting container fake -
...
- [ ] 5.2 `tests/test_axi_logs.py`: the verb emits one TOON document (metadata + per-process blocks),
...
- [ ] 5.3 `tests/test_axi_logs.py`: assert `logs` IS registered in `axi_mod.app.registered_commands`
      (design Decision 4 - the presence assertion closing g4's logs half).
- [ ] 5.4 Assert the shared resolve did not regress `cwcli logs`: the step-1 five regressions stay green.

## 6. Skill + docs

- [ ] 6.1 Regenerate the installable skill (`build_skill.py`); confirm `axi logs` appears in the verb
      table with no hand-edit, and `tests/test_axi_skill.py --check` passes.
- [ ] 6.2 Update `README.md`'s agent-surface section and `CLAUDE.md`'s logs entry: `axi logs` now ships
      (a bounded `core.read_logs`, no follow); note the g4 logs-half closure.

## 7. Gates

- [ ] 7.1 `uv run pytest` (fast tier), `uv run black --check src/`, `uv run ruff check src/`,
