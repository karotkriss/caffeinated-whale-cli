...
- [x] 6.4 Re-run AFTER no-mistakes and after any review fixes (the standard: review fixes are a trigger, not an exemption). Done 2026-07-16: after both no-mistakes runs (including the rebase onto post-#94 develop, the document-step doc fixes, and the lint reformat), the full 15-leg E2E was re-run at the rebased head against the same throwaway instance - 15/15 green, fast tier 1184 + mypy zero at that head.
