...
- **Unchanged:** `utils/config_utils.py` / `utils/auto_inspect.py` / `utils/startup.py` mechanics (storage and process layers; the two auto-inspect entries on the Known-hazards board - PID reuse, SIGTERM re-entry - are deliberately NOT absorbed, per the board's own discipline), `db_utils` cache mechanics, every non-config command, the TOML schema (no config-file migration needed).
  ONE disclosed hardening landed in `utils/auto_inspect.py` during E2E (2026-07-16): the POSIX fork child now `dup2`s its REAL fds (0/1 to devnull, 2 to the log file) instead of only rebinding `sys.*` - without it the detached daemon held the parent's stdout pipe open and the fused `enable` hung every `$(...)`/pipe capture. The old surface masked the bug (`enable` never started a daemon); the rework made it load-bearing. Evidence + the pinning test: `docs/e2e/config-dx-rework.md`, `tests/test_auto_inspect.py::test_forked_daemon_releases_the_parents_stdout_pipe`. The two Known-hazards entries remain untouched.
