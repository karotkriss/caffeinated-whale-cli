...
- [ ] 5.5 **Measure** the instance-wide latency on a real 2-bench and 6-bench instance and record the numbers (`design.md` Decision 7). NOT DONE: it needs a real multi-bench Frappe instance, and only one real Frappe E2E may run on this box at a time. The hoist is NOT done either, which is the correct default - optimizing a cost nobody has measured is how a small change acquires a second mechanism.
...
The committed E2E suite is UPDATED for the new behaviour (CI owns running it; only one real Frappe E2E may run on the build box at a time, so it was not hand-run here):
...
- [ ] 7.1/7.2/7.3 A genuine two-bench instance with BOTH benches really serving (real ports 8000/8001, both under supervisord) - the runtime proof of F3/F4 against the FIXED build. **NOT DONE and deliberately not faked:** no existing harness fixture builds a second SERVING bench (the skeleton above has no venv, no Procfile, no supervisord), and standing one up means starting a real Frappe instance, which the build box permits only one of at a time. The defects themselves were already proven at runtime by the audit; the fix is pinned at unit level by `TestPerBenchWebProbe` against two-bench fakes carrying per-bench `ps`, per-bench `supervisorctl`, and per-PORT curl answers. This is the residual gap, stated rather than papered over.
- [ ] 7.6 Teardown by exact name; no broad prune; no protected instance touched. (Applies when 7.1-7.3 are run.)
