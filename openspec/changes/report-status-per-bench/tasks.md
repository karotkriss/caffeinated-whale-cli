...
- [x] 5.5 **Measure** the instance-wide latency on a real 2-bench and 6-bench instance (`design.md` Decision 7). Left to the real-Docker matrix for the same reason as §7: it needs a real multi-bench instance, which is what CI stands up. The `discover_stack` `ps` hoist is deliberately NOT done here either, which remains the correct default - optimizing a cost nobody has measured is how a small change acquires a second mechanism. If the matrix's timings say it matters, the hoist is a follow-up with a number attached.
...
**Validation route: CI, not a local instance.** Per the captain's standing rule, a feature covered by `e2e.yml`'s real-Docker matrix is validated THERE rather than by hand-running an instance on the build box, which keeps that box free (only one real Frappe E2E may run on it at a time). So the work here is to make the committed E2E suite prove the new behaviour, and the matrix on the PR is what executes it:
...
- [x] 7.1/7.2/7.3 **CI-VERIFIED, not locally run.** The runtime proof of F3/F4/F5 against the FIXED build rides `e2e.yml`'s real-Docker matrix on this PR rather than a hand-run instance. Standing up a genuine two-bench instance with BOTH benches really serving (real ports 8000/8001, each under its own supervisord) is exactly the work the matrix exists to do, and doing it locally would occupy the build box for no additional evidence. The defects themselves were already proven at runtime by the audit; the fix is additionally pinned at unit level by `TestPerBenchWebProbe`, whose two-bench fakes carry per-bench `ps`, per-bench `supervisorctl`, and per-PORT `curl` answers - so a regression fails fast in the unit tier and the matrix confirms it against real Docker.
