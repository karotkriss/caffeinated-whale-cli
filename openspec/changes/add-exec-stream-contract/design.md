...
- **Decision (as proposed).** `RunPlan` carries `container_id: str`, and `exec_stream` takes a `container_id` and re-resolves internally.

> ### CORRECTION, made during implementation. The second half of this decision was WRONG.
>
> **`RunPlan.container_id: str` is right and shipped.** A plan crosses a return boundary; the rule applies.
>
> **`exec_stream(container_id, ...)` was wrong**, and it over-applied the very rule this decision was reasoning about. Locked decision 4 governs what a `core.<verb>` **RETURNS**. It says nothing about parameters, and the foundation already ships primitives that **accept** live containers - `resolvers.resolve_container_state(project, frappe_container, ...)` and `resolvers.require_bench_dir(frappe_container, ...)`. By making the primitive take an id, I forced it to construct its own Docker client, which discarded the client every caller already held and, more tellingly, **broke 24 tests in `tests/test_apps.py`** whose fakes supply a live container. The proposal promised that suite green *unchanged* as the refactor's proof; the proof did its job and rejected the design.
>
> **What shipped:** `exec_stream(container, cmd, *, workdir, environment)` takes a live container and uses `container.client.api`, matching the existing primitives. `core/docker.py:get_container(container_id)` is the id-to-handle bridge, and `core.run_stream(plan) -> Iterator[ExecEvent]` is phase 2, calling it. So the id is still load-bearing (the exec lands on the container that was PLANNED, not on a re-resolution by project name), the frontend still never touches a container, and `apps`/`update` pass the container they already hold.
>
> **The cost of one new core accessor** (`get_container`, 8 lines, in the module where container accessors live) buys back the double-resolve this decision was right to reject. Reported rather than absorbed, per the falsifiable-claim standard: the resolver primitives came back **zero bent**, but this batch's own new primitive needed correcting, and an existing test suite is what caught it.

- **Rejected: `exec_stream(container, ...)` taking the live object, with `commands/run.py` resolving it.** The frontend would resolve the container a second time (`run_plan` just did), which is the double-resolve the plan exists to prevent, and it pushes container resolution back into a frontend the batch is trying to thin. **This rejection still stands** - the correction above keeps the bridge inside the core (`run_stream`), not in the frontend.
- **Rejected: a module-level cached client.** Over-engineering against a cost the codebase already pays everywhere; if client churn ever matters it is one change in `utils/docker_utils`, benefiting every command rather than this one. **Moot after the correction:** the primitive builds no client at all.
