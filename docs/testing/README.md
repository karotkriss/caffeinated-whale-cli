...
Measured with `uv run pytest --cov` at 0.37.0 (unreleased): 1183 tests across 71 test files, ~70% overall coverage.
...
Run `ls tests/` for the authoritative, current list; as of 0.37.0 (unreleased) it holds 71 `test_*.py` suites plus `bench_fakes.py` and `bench_fakes_mb.py` (shared fakes) and `README.md`.
...
5. **Real-Docker E2E for `init` and `backup`** (`tests/e2e/test_init_e2e.py`, `tests/e2e/test_backup_e2e.py`) - genuine `bench init`/`bench backup` against throwaway Frappe instances on the v14/v15/v16 matrix, both interactive and non-interactive. The remaining commands (`rm`, `restore`, `inspect`, `apps list`/`install`/`uninstall`) and the P2P loopback are deferred to follow-up PRs (`openspec/changes/rebuild-e2e-test-suite`); `unlock` closed its own gap (item 6d) and `update`/`apps update` closed theirs (item 6e) - `apps`'s other three subcommands moving onto the core (item 6f) is unit-tier only and does not close this gap, by design.
