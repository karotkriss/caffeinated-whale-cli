...
Measured with `uv run pytest --cov` at 0.37.0 (unreleased): 951 tests across 58 test files, ~65% overall coverage.
...
- **app management** (`test_apps`) - `commands/apps.py` ~91% (multi-site fan-out, frappe reset, `update` deprecation)
...
Run `ls tests/` for the authoritative, current list; as of 0.37.0 (unreleased) it holds 58 `test_*.py` suites plus `bench_fakes.py` and `bench_fakes_mb.py` (shared fakes) and `README.md`.
...
3. **App Management** (`commands/apps.py`, `commands/update.py`) - covered by `test_apps` (`commands/apps.py` ~91%); `commands/update.py`'s state machine moved to `core/update.py` (~90%), the reseated renderer is ~88%, see item 6e.
