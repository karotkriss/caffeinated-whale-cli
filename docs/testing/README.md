...
- **`logs` onto the logic core, plus `cwcli axi logs`** (`test_core_logs`, `test_logs`, `test_axi_logs`) - `core/logs.py` 100% (the resolve migrated off `commands/logs.py`: the combined/`--process` log selection, the `select_bench`/`confirm_start`/`select_process` `NEEDS_CHOICE` forks, the two distinct no-logs errors - `logs.none_yet` NOT_FOUND when a manager is up vs `logs.no_manager` NOT_RUNNING with the start hint - the not-cwcli-supervised fallback firing and not firing, and the `subprocess`-ban purity check; since `add-axi-logs-verb` the resolve is shared via `_resolve_log_files` with the bounded `read_logs` behind the new `axi logs` verb, whose running-but-quiet bench reads as an empty success rather than `logs_plan`'s error), the reseated `commands/logs.py` renderer ~80% (the `--follow` default flip to `False`, gating `docker exec -it` on `sys.stdin.isatty()` so a non-TTY/piped invocation never requests a TTY, and PR #83's exit-code fix pinned through the public surface), and `commands/axi.py`'s `logs` verb (one TOON document - a metadata head then a raw-line block per process - the exit-code mapping, and the registry-presence assertion)
...
**No dedicated suite** (only incidental coverage from other tests' mocking): `utils/port_utils.py` (~9%), `utils/vscode_utils.py` (~15%, down from ~16% after the dead editor-selection helpers were deleted in `migrate-open-core`).
`utils/docker_utils.py` remains only incidentally covered via `test_core_resolvers`'s `get_frappe_container` CLI wrapper. `decode_exec_stream` is GONE - `core/exec_stream.py` superseded it and its only three callers were re-pointed at that primitive. The UI-free primitives `utf8_stream_decoder`/`get_project_containers`/`get_project_volumes` MOVED to `core/docker.py` so the UI-pure core never imports this UI module - `test_exec_stream_decode` dedicated-tests `utf8_stream_decoder` there now.
`utils/sendme_utils.py` is no longer in this bucket: `test_sendme_utils` lifted it to ~89% (see [tests/README.md](../../tests/README.md#test-coverage) for the breakdown).
**Target**: add dedicated suites for the remaining two modules next.
...
5b. **Real-Docker E2E for the P2P (`sendme`) loopback** (`tests/e2e/test_restore_p2p_e2e.py`) - fills the `e2e_p2p` marker with a real send->receive transfer: `restore --send` produces a genuine sendme ticket on a wide pty, non-interactive `restore --receive --ticket <t>` pulls-and-restores it, proven by an ORIGINAL/MUTATED DB marker; runs on its own dedicated instance (never the shared session one) so a receive DB-wipe cannot corrupt another test's fixture, plus a separate v14-gated case for the `--receive` bare-filename bug. `tests/test_sendme_utils.py` lifted `utils/sendme_utils.py` from ~9% to ~89% alongside it (see [tests/README.md](../../tests/README.md#test-coverage)).
...
### Still needed
8. **Docker Utilities** (`utils/docker_utils.py`) - foundation for all commands; the `get_frappe_container` CLI wrapper is covered by `test_core_resolvers`, but the rest of the module's error handling is untested by a dedicated suite. The exec-stream decode helper (`utf8_stream_decoder`) has since moved to `core/docker.py` and is dedicated-tested there by `test_exec_stream_decode`.
