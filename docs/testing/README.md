...
Measured with `uv run pytest --cov` at 0.37.0 (unreleased): 780 tests across 50 test files, ~62% overall coverage.
...
Run `ls tests/` for the authoritative, current list; as of 0.37.0 (unreleased) it holds 50 `test_*.py` suites plus `bench_fakes.py` and `bench_fakes_mb.py` (shared fakes) and `README.md`.
