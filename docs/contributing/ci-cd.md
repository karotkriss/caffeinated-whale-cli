...
- **`latency_benches` dispatch input (opt-in, off by default):** sets `CWE2E_LATENCY_BENCHES` for the run, which is what selects `tests/e2e/test_multibench_latency_e2e.py` - the instance-wide `cwcli status` latency measurement (`report-status-per-bench` task 5.5). Set it to `6` to grow a real six-bench serving instance on the v16 `standalone` leg and record the per-bench cost curve. It is off on every PR because each data point is a genuine bench build (roughly 1.5 GB and a couple of minutes apiece), which is far too much to spend per pull request on a number that only needs retaking when the probe changes. Setting it also raises the job's `timeout-minutes` to 90, since the extra bench builds land on top of the ordinary leg - measured at 29 minutes for the whole v16 `standalone` group plus a six-bench instance. Disk is not a constraint despite the bench trees: that same run finished with 108 GB free. Recorded numbers live in [`docs/e2e/multibench-serving-status.md`](../e2e/multibench-serving-status.md).
