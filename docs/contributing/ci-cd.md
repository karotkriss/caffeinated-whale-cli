...
- **Trigger gate:** a manual dispatch, a PR carrying the `e2e` label, or a PR whose base branch is `develop`/`master`. The `E2E (frappe vNN)` and `E2E (runtime-only install)` checks are required by `develop` branch protection and block merges.
