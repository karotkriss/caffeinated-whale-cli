# Documentation

This directory contains documentation for the caffeinated-whale-cli project.

## Contents

### Project Documentation

- **[bench.md](./bench.md)** - Bench management documentation

### CI/CD Documentation

- **[ci-cd/overview.md](./ci-cd/overview.md)** - GitHub Actions workflows implementation overview
- **[ci-cd/setup-guide.md](./ci-cd/setup-guide.md)** - Complete setup guide for GitHub Actions
- **[ci-cd/linting.md](./ci-cd/linting.md)** - Code quality and linting tools
- **[ci-cd/dependency-management.md](./ci-cd/dependency-management.md)** - Managing dependencies with uv

## Quick Links

### Getting Started

1. [Main README](../README.md) - Project overview and installation
2. [CHANGELOG](../CHANGELOG.md) - Version history and changes

### Release Process

1. [CI/CD Overview](./ci-cd/overview.md) - Learn about automated workflows
2. [Setup Guide](./ci-cd/setup-guide.md) - Configure GitHub Actions for releases
3. [Release Notes Template](../RELEASE_NOTES_0.9.1.md) - Example release notes

### Development

1. [Contributing Guidelines](./ci-cd/setup-guide.md#testing-strategy) - Testing workflows
2. [Workflow Quick Reference](../.github/workflows/README.md) - Daily workflow operations

## Directory Structure

```
docs/
├── README.md                    # This file
├── bench.md                     # Bench management docs
└── ci-cd/
    ├── overview.md              # Workflows implementation
    └── setup-guide.md           # GitHub Actions setup
```

## External Resources

- [GitHub Repository](https://github.com/karotkriss/caffeinated-whale-cli)
- [Issues](https://github.com/karotkriss/caffeinated-whale-cli/issues)
- [PyPI Package](https://pypi.org/project/caffeinated-whale-cli/)
