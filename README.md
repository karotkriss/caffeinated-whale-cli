...
`backup`, `unlock`, `stop`, `label`, `run`, `ls`/`list`, `where`, `start`/`status`/`restart`, `logs`, `inspect`, `apps`, and `update` are migrated onto it so far; `restore`, `rm`, `config`, and `init` are not yet.
