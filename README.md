...
**`apps install`** - fetches (`bench get-app`, honoring `--branch`) and installs each app on the target site(s). Each `APP` is a known app name **or** a git URL (passed straight to `bench get-app`, so custom apps not in bench's registry work). `--fetch-only` fetches without installing on any site. A bench that is already running is serving code it loaded before the install, so it cannot see the new app: install therefore restarts that bench's supervised web process and requires every changed site to answer Frappe before it reports success. The restart is announced as it happens and reported as a `restart-web` row, and the command exits non-zero rather than claiming success for a site it could not confirm. Only the `web` process is cycled, so a background job that needs the new app may still want a full `cwcli restart`; a bench that was not running is left alone.

**`apps uninstall`** - removes each app from the target site(s) (`bench --site <site> uninstall-app`). This destroys site data, so it is gated by `-y`/`--yes` or an interactive confirmation (a non-TTY without `--yes` refuses). A running bench gets the same announced, verified `restart-web` step after an uninstall, so the old process cannot keep serving the code and tables of an app that is gone.
