...
Dirty means precisely **staged changes and unstaged modifications to tracked files**; **untracked files are not dirty** and never block the checkout, because nothing here removes them.
`--reset` is the explicit opt-in through that refusal: it hard-resets the working tree to the fetched ref, discarding those local edits (untracked files still survive), which guarantees the clean tree a subsequent `build`/`migrate` needs.
